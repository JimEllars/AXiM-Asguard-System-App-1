...
// Update Triage submission
const oldTriageHandler = `  const handleTriageAnomaly = async (action: 'block' | 'dismiss') => {
...
const newTriageHandler = `  const handleTriageAnomaly = async (action: 'block' | 'dismiss') => {
...
code = code.replace(oldTriageHandler, newTriageHandler);


const oldBanner = `          {anomalyQueue?.status === 'pending_onyx_triage' && (
            <div
...
const newBanner = `          {anomalyQueue.length > 0 && (
            <div
              onClick={() => setShowTriageModal(true)}
              className="mb-6 border border-amber-500/50 bg-amber-500/10 rounded font-mono p-4 text-center text-sm text-amber-500 cursor-pointer hover:bg-amber-500/20 transition-colors animate-pulse"
            >
...
const oldModal = `        {showTriageModal && anomalyQueue && (
...
const newModal = `        {showTriageModal && anomalyQueue.length > 0 && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
