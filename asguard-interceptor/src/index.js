...
function getCorsHeaders(request, env, isMutation) {
    ...
    return {
        ...
        ...(request.aiDuration ? { "Server-Timing": `ai-eval;dur=${request.aiDuration};desc="Llama Guard 3 8B"` } : {}),
    };
}
async function evaluateEdgeSafety(env, inputContent) {
    if (!env.AI)
        return { safe: true, threatCategory: null, aiDuration: 0 };
...
async function dispatchCriticalAlert(env, eventPayload, request, ctx) {
    try {
        if (!env.ASGUARD_ALERT_WEBHOOK_URL)
            return;
...
async function runMaintenanceSweep(env, ctx, sweepType) {
    ...
    if (isDaily) {
        ...
        try {
            ...
            const recentEventsStr = await env.ASGUARD_TELEMETRY.get("recent_events", { type: "json" }) || [];
            const recentEvents = Array.isArray(recentEventsStr) ? recentEventsStr : [];
            totalIntercepted24h = recentEvents.filter((event) => event.timestamp && event.timestamp >= twentyFourHoursAgo).length;
            aiThreatCount24h = recentEvents.filter((event) => event.aiThreatFlag === true &&
                event.timestamp &&
                event.timestamp >= twentyFourHoursAgo).length;
            const appOriginBreakdown24h = {};
...
export default {
    ...
    async fetch(request, env, ctx) {
        ...
        if (request.aiDuration) {
            serverTiming += `, ai-eval;dur=${request.aiDuration};desc="Llama Guard 3 8B"`;
        }
    ...
    async handle(request, env, ctx) {
                ...
                ctx.waitUntil((async () => {
                    try {
                        const auditDbOp = async () => {
                            ...
                            await env.ASGUARD_TELEMETRY.put("recent_events", JSON.stringify([{
                                    timestamp: timestamp,
                                    eventType: "audit_log",
                                    severity: "low",
                                    sourceIp: "internal",
        ...
        if (request.method === "POST" && url.pathname === "/dlq/bulk-purge") {
            ...
            try {
                ...
                let purged = 0;
                let failed = 0;
                const purgePromises = ids.map(async (record) => {
                    let targetKvKey = '';
                    ...
                    try {
                        await Promise.all([
                            env.ASGUARD_TELEMETRY.put(`audit:${timestamp}-${Math.random()}`, JSON.stringify({
            ...
            try {
                ...
                if (contentToEvaluate) {
                    ...
                    if (aiSafety.aiDuration) {
                        request.aiDuration = (request.aiDuration || 0) + aiSafety.aiDuration;
                    }
                    if (!aiSafety.safe) {
        ...
        if (request.method === "POST" && url.pathname === "/telemetry") {
            try {
                ...
                if (request.cf && request.cf.botManagement && request.cf.botManagement.score !== undefined) {
                    payload.botScore = request.cf.botManagement.score;
                    if (!payload.details)
                        payload.details = {};
