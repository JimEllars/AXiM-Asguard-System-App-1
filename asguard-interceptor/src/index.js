...
function hasSecret(request, header, expected) {
...
function isAdminRequest(request, env) {
    return (hasSecret(request, "X-Asguard-Auth", env.ASGUARD_API_KEY) ||
        hasSecret(request, "X-Asguard-Service-Token", env.ASGUARD_SERVICE_TOKEN));
}
async function analyzeThreat(payload, env) {
    ...
    const providers = [
        {
            ...
            request: () => fetch("https://api.deepseek.com/chat/completions", {
                method: "POST",
                headers: {
        ...
        },
        {
            ...
            extract: (body) => body.content?.find((part) => part.type === "text")?.text,
        },
    ];
