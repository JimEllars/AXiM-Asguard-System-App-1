...
// .wrangler/tmp/bundle-Fduykk/checked-fetch.js
...
var TelemetryPayloadSchema = external_exports.object({
  sourceIp: external_exports.string().ip(),
  // z.string().ip() works with zod 3.22.0
  timestamp: external_exports.number(),
  eventType: external_exports.enum(["authentication_failure", "signature_tampering", "suspicious_activity", "client_error"]),
  severity: external_exports.enum(["low", "medium", "high", "critical"]),
  requestMethod: external_exports.string().optional(),
  targetResource: external_exports.string().optional(),
  signatureMetadata: external_exports.string().optional(),
  details: external_exports.record(external_exports.unknown()).optional(),
  country: external_exports.string().optional(),
  colo: external_exports.string().optional()
});

// src/index.ts
var rateLimitMap = /* @__PURE__ */ new Map();
var penaltyLedger = /* @__PURE__ */ new Map();
var clientErrorThrottleMap = /* @__PURE__ */ new Map();
function pruneRateLimitMap() {
  const now = Date.now();
  if (rateLimitMap.size > 1e4) {
    for (const [key, value] of rateLimitMap.entries()) {
      if (now - value.timestamp > 1e4) {
        rateLimitMap.delete(key);
      }
    }
  }
  if (penaltyLedger.size > 1e3) {
  ...
  if (clientErrorThrottleMap.size > 1e4) {
    for (const [key, timestamps] of clientErrorThrottleMap.entries()) {
      const valid = timestamps.filter((t) => now - t <= 1e4);
      if (valid.length === 0) {
...
__name(pruneRateLimitMap, "pruneRateLimitMap");
...
var src_default = {
  ...
  async handle(request, env, ctx) {
    ...
    if (request.method === "OPTIONS") {
      const headers2 = getCorsHeaders(request, env, true);
      if (!headers2["Access-Control-Allow-Origin"]) {
    ...
    if (clientIp !== "unknown") {
      ...
      if (record && now - record.timestamp <= 1e4) {
        record.count++;
      } else {
      ...
      if (currentCount > 10) {
        ...
        if (penalty && now - penalty.timestamp <= 6e4) {
          penalty.consecutive++;
    ...
    if (request.method === "GET" && url.pathname === "/health") {
      ...
      try {
        await Promise.all([
          env.ASGUARD_BLACKLIST.get("health-check-key").catch((e) => {
            throw new Error("ASGUARD_BLACKLIST failed");
          }),
          env.ASGUARD_TELEMETRY.get("health-check-key").catch((e) => {
            throw new Error("ASGUARD_TELEMETRY failed");
          })
        ]);
    ...
    if ((request.method === "POST" || request.method === "DELETE") && url.pathname === "/blocklist") {
      ...
      try {
            ...
            (async () => {
              try {
                await Promise.race([
                  ...
                  new Promise((_, r) => setTimeout(() => r(new Error("Timeout")), 5e3))
                ]);
              } catch (e) {
        ...
        const timestamp = Date.now();
        const ttl = payload.action === "block" ? payload.ttl || 86400 : void 0;
        ctx.waitUntil(
          (async () => {
            try {
              const auditDbOp = /* @__PURE__ */ __name(async () => {
                await env.ASGUARD_TELEMETRY.put(
                  `audit:${timestamp}`,
                  JSON.stringify({
              ...
              const auditTimeout = new Promise(
    ...
    if (request.method === "POST" && url.pathname === "/telemetry/client-error") {
      ...
      try {
        ...
        const payload = {
          sourceIp: clientIp,
          // Use standard client IP from connection
          timestamp: rawPayload.timestamp || Date.now(),
          eventType: "client_error",
          severity: "medium",
          // Default to medium severity for client errors
...
async function logTelemetry(data, env) {
  try {
    ...
    const dbOp = /* @__PURE__ */ __name(async () => {
      ...
      if (mutationErrors.length > 0) {
        await Promise.all(mutationErrors.map(async (errFrame) => {
          await env.ASGUARD_TELEMETRY.put(`dlq:${Date.now()}-${Math.random()}`, JSON.stringify({
            ...
            timestamp: Date.now(),
            originNode: "UNKNOWN",
            // Could be enriched if available
...
// .wrangler/tmp/bundle-Fduykk/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Fduykk/middleware-loader.entry.ts
